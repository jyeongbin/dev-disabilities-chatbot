...
# text-embedding-ada-002 입력 가능한 토큰수는 최대 4096
...
# --- 아래 프롬프트 정의 추가 ---
# 검색기(Retriever)가 대화 기록을 보고 질문을 재구성하는 프롬프트
...
# 1. History-Aware Retriever 생성: 대화 기록 + 새 질문 -> 검색용 질문 생성
history_aware_retriever = create_history_aware_retriever(
    model, retriever, retriever_prompt
)

# 2. Document Chain 생성: 검색된 문서 + 원본 질문 -> 답변 생성
document_chain = create_stuff_documents_chain(model, retriever_prompt) # retriever_prompt 재사용 가능

# 3. Retrieval Chain 생성: 위의 두 체인을 연결
...
# --- 아래 프롬프트 정의 추가 ---
        ...
        retriever = vectordb.as_retriever()
        #chain = ConversationalRetrievalChain.from_llm(model, retriever, return_source_documents=True)

# --- 아래 새 Chain 생성 코드로 대체 ---
        history_aware_retriever = create_history_aware_retriever(
            model, retriever, retriever_prompt
        )
        document_chain = create_stuff_documents_chain(model, retriever_prompt)
        new_chain = create_retrieval_chain(history_aware_retriever, document_chain)
        # --- 대체 끝 ---

        # chain 을 chain_queue에 저장
        chain_queue.put(new_chain)
